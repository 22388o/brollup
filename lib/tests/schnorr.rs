#[cfg(test)]
mod schnorr_tests {
    use brollup::signature::{into::IntoBytes, schnorr::{schnorr_sign, schnorr_verify, SecpError, SignFlag}};
    ...
    fn test_sign_schnorr() -> Result<(), SecpError> {
        ...
        let sig = schnorr_sign(
            private_key.into_secret_key_bytes()?,
            message.into_message_bytes()?,
            SignFlag::EntrySign,
        )?;
    ...
    fn test_verify_schnorr() -> Result<(), SecpError> {
        ...
        schnorr_verify(
            public_key.into_public_key_bytes()?,
            message.into_message_bytes()?,
